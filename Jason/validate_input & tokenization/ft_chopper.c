...
int	count_word(const char *str, int n_word, int flag_inword, int flag_inquote)
{
	while (*str)
	{
		if (*str == ' ' && flag_inquote == 0)	// Case(1): meet with "space"(reference point for dividing word)
			flag_inword = 0;
		else if (*str == '=' || *str == '<' || *str == '>'	// Case(2): meet with "redirection char"
			|| *str == '|' || *str == '$')
		{
			n_word++;
			flag_inword = 0;
		}
		else if (*str == '"' || *str == '\'')	// Case(3): meet with "s_quote" or "d_quotes"
			toggle_inword_inquote(&flag_inword, &n_word, &flag_inquote);
		else									// Case(4): meet with "everything else"
		{
			if (flag_inword == 0)
			{
				n_word++;
...
typedef struct s_data
{
	int		n_word;		-> [func]"count_word"	-> how many words
	int		n_sq;		-> [func]"count_quote"	-> how many single quote
	int		n_dq;		-> [func]"count_quote"	-> how many double quotes
	char	**token;		-> [malloc]		-> Array for each words (first dimension)
	char	*str;			-> [param]		-> Input from user.
}	t_data;
*/
char	**validate_input(char *user_input, char **env)
{
	...
	data.str = user_input;						// cpy the pointer to "user_input" // (ex) "ls -l | cat -e"
	if (count_quote(user_input, &data) == -1)			// count s_q & d_q from "user_input"
		return (NULL);						// If n_sq || n_dq isn't even number -> validate fails
	if (data.n_word = count_word(data.tmp, 0, 0, 0) == 0)		// Count the number of "word" & save it in "data" struct
		return (NULL);
	if (data.n_sq + data.n_dq > 0)					// if "n_sq" or "n_dq" is in case of one or more.
		if (check_order_quote(user_input, &data, 0, 0) == -1)	// Check the order of quotes and pair.
			return (NULL);					// If the order of quote doesn't have any problem, nothing change.
	data.token = malloc((data.n_word + 1) * sizeof(char *));	// Allocate memories for each word. token[0]: 1st word, token[1]: 2nd word ..
	if (data.token == NULL)
		return (NULL);
	if (ft_chopper(&data, user_input, 0) == -1)			// Save all words to 2D-array in original order.
		return (NULL);
	if (expansion(&data, env, 0) == -1)				// Check ($) dollar sign and comparing with env variable
		return (NULL);
	return (data.token);						// Now we are storing each separate token that passed validation in (t_data data)
}
