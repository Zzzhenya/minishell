...
/*	[F]
	[Form]
	data->token[i] = replace_str(data->token[i],
...
*/
char	*replace_str(char *token, char *row_env, int i_dollar)
{
	int		i;
	int		i_new;
	char	*res;
...
int	expand_env(t_data *data, char **env, int i)
{
	...
	while (data->token[i] != NULL)
	{
			...
			&& data->token[i][i_dollar + 1] != '$')
		{
			...
			data->token[i]
				= replace_str(data->token[i], env[row_env], i_dollar);
			if (data->token[i] == NULL)
				return (-1);
		}
